---
title: "Datalayers RBAC 认证与授权"
description: "介绍 Datalayers 的 RBAC 认证与授权模型，包括初始化方式、配置项和多用户细粒度权限控制能力。"
---
# Datalayers RBAC 认证与授权

## 概述

RBAC 提供完整的身份认证和权限管理体系，支持多用户、多角色和细粒度权限控制。该模式适用于生产环境、多团队共享环境以及需要最小权限治理的场景。

## 核心特性

- 角色管理：定义不同职责和权限级别的角色
- 用户分配：将用户授予到指定角色
- 权限控制：精确控制数据库、表和操作级别的访问权限
...
安装 Datalayers 后，首次以 `rbac` 模式启动时，系统默认没有预置管理员账户，因此需要手动初始化第一个管理员用户。本文提供两种初始化方式。
...
- 参考 `rbac` 中的 [用户管理](../rbac/user.md)，并创建好账号
...
- 了解静态认证方式，请参考 [Datalayers 静态认证](./static.md)
- 了解密码策略，请参考 [Datalayers 密码策略](../password-policy.md)
