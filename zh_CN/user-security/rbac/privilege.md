---
title: "Datalayers 权限管理"
description: "介绍 Datalayers 的权限类型、作用域、授权与回收语法，帮助你构建可控的数据库访问策略。"
---
# Datalayers 权限管理

权限用于定义用户或角色可以执行的操作范围，是 RBAC 模型中的核心组成部分。通过合理设计权限作用域和授权方式，可以实现最小权限原则和安全隔离。
...
- 了解用户创建与管理，请参考 [用户管理指南](./user.md)
- 了解角色定义与授权，请参考 [角色管理指南](./role.md)
