...
|  FLUSH_INTERVAL       | 每间隔多长时间自动将内存数据持久化到文件中，缺省值为1d。如关闭则设置为`0`。不支持单位：m（分钟）、h（小时）、d（天）                                    |  
...
| COMPACT_MODE               | Compaction 支持的模式，缺省值为开启所有支持，支持选项：COMPACT, TTL, DELTA，选项可以并列，以英文逗号分隔  |
