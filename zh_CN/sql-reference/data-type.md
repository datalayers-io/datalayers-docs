...
| [DECIMAL](./decimal.md) | DEC、NUMERIC | 精确定点数，最大 precision 为 76 |
...
| 类型 | 说明 |
| --- | --- |
| ARRAY | `ARRAY(element_type)`,通过 `element_type`指定数据类型 |
| STRUCT | `STRUCT<field_name field_type, ...>`, `file_name` 指定元素名字，`filed_type` 指定该类型类型 |
