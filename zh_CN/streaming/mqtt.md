...
description: "介绍 Datalayers 流计算中 MQTT source connector 的配置项、适用场景和完整使用示例。"
---

# MQTT Connector

MQTT connector 用于订阅 MQTT broker 中的主题消息，并将其持续读入 Datalayers source。
...
- `2` 或 `exactly_once`：恰好一次。语义最强，但开销更高。
...
```bash
docker run -d --name dl-mqtt \
  -p 1883:1883 \
  -v "$(pwd)/.github/e2e-config/mosquitto.conf:/mosquitto/config/mosquitto.conf:ro" \
  eclipse-mosquitto:2
```
