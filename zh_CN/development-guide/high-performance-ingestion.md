---
title: "高性能写入"
description: "Datalayers 高性能写入指南：介绍时序场景下批量写入、行协议写入与参数优化等最佳实践，帮助你提升写入吞吐与稳定性。"
---
# 高性能写入

本文介绍在时序数据场景下实现高效写入 Datalayers 的最佳实践，涵盖影响写入性能的关键因素与优化策略。
...
在时序模型中，数据首先被写入到内存中（即：memtable），当内存中数据达到设置的阈值的一半时（即：memtable_size），将会触发落盘的行为，将内存中的数据转存到持久化存储中。后台线程再根据策略，对落盘的文件进行合并、整理，如果 memtable size 设置较小，会导致频繁落盘、compact，从而影响读写性能。
