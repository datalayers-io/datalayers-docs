...
- **流计算**
  - 支持定义 HTTP、Kafka 和 MQTT 数据源
  - 数据源支持 CSV、JSON 和 Parquet 三种数据格式
  - 支持创建 Pipeline，可对数据源中的数据进行过滤后写入时序表
  - 新增 `SHOW PIPELINES` 语句和系统表 `information_schema.pipelines`，便于查看当前已创建的 Pipeline
  - 新增 `SHOW SOURCES` 语句和系统表 `information_schema.sources`，便于查看当前已创建的数据源
  - 新增 `SHOW CREATE SOURCE` 语句，用于查看 Source 的创建语句
  - 新增 `SHOW CREATE PIPELINE` 语句，用于查看 Pipeline 的创建语句

### 增强

- 支持客户端认证链校验
- 新增 `CREATE TABLE AS` 语法，支持基于查询结果直接建表
- 新增 `STRUCT` 和 `ARRAY` 数据类型

### 修复

- 修复使用 JDBC 驱动写入时无法获取受影响行数的问题
- 修复部分 Postgres 客户端连接兼容性问题
- 当配置文件时区设置为 UTC 时，查询结果中的时间列不再进行时区转换
- 日志时间统一调整为 UTC
