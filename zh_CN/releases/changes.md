...
- 新增支持 `current_database()` 函数。

### 问题修复

- 修复 PostgreSQL 协议下执行 `SET` 语句时，服务端未返回空结果的问题。
...
- 支持配置日志保留数量，过期日志自动清理，避免存储空间持续占用。
- 系统默认最大 flush 间隔现可通过配置文件调整。
- 优化 PostgreSQL 连接协议，新增连接时 `currentSchema` 配置，支持指定默认 Schema。
...
- TLS 配置优化：支持为各类服务端口独立配置 TLS。
- 控制台升级：升级内置 Console Frontend，提升控制台使用体验与前端能力。

### 性能优化

- 向量检索增强：支持 `HNSW_RQ`，并优化 RQ 量化实现，在向量索引构建与检索场景中提升性能与能力边界。
...
- 对象存储兼容性提升：支持阿里云 IMDS 认证；支持在对象存储中配置目录前缀。
- DDL 能力增强：`ALTER TABLE` 现支持在一次语句中执行多个变更操作，提升表结构调整效率。
- PostgreSQL 安全连接增强：PostgreSQL 协议支持 TLS，提升接入过程中的传输安全性。
- 全文检索能力增强：进一步完善基于 Tantivy 的全文检索实现，增强全文查询能力。

### 性能优化

- 自适应并行优化：新增 Adaptive Parallelism，按查询场景动态调整并行度，提升资源利用率与执行效率。
- 集群聚合优化：集群模式支持 Two-Phase Aggregation，降低跨节点聚合开销，提升分布式查询性能。
- 优化器增强：优化分区键过滤与冗余过滤消除逻辑，进一步提升查询计划质量。

### 问题修复

- 缓存与写入修复：修复 Last Cache 缺陷，并修复 Prometheus 无 Schema 写入处理不正确的问题。
- 键列处理修复：修复分区键、实体键与主键在部分场景下使用不正确的问题。
...
- 集群许可证可视化增强：支持在集群中查看所有节点的 License 信息，便于统一排查节点授权状态。
- 可观测性增强：新增 FDB Backup 状态，Datalayers 网络吞吐、磁盘 IOPS 等指标，提升运行状态的可观测性。

### 性能优化

- 最新值查询优化：优化主键 latest query 与 latest-k 查询路径，进一步提升最新点查询性能。
- 查询执行优化：完善基于统计信息的聚合预计算、文件裁剪、冗余过滤消除、Repartition 裁剪等优化策略。
- 下推能力增强：支持更多窗口函数下推，并优化排序下推到 Parquet Scan 以及按主键时间列排序场景的执行效率。
- 元数据处理优化：优化对象元信息构建逻辑，降低 IO 次数，提升查询性能。
...
- 查询 Hint 增强：新增 hybrid_cache Hint，支持按查询粒度控制是否启用 Hybrid Cache，便于在性能与资源占用之间做更细粒度权衡。

### 问题修复

- 修复 last cache 更新不正确的问题，确保最新值缓存结果准确。
...
- Last Cache 可观测性增强：新增命中率与未命中指标，便于分析缓存效果与查询收益。

### 问题修复

- PostgreSQL 协议兼容性修复：调整部分整数类型映射，并完善时间戳处理逻辑，提升 PostgreSQL 客户端兼容性。
...
- **流计算能力**
  - **多数据源适配**：新增支持 HTTP、Kafka、MQTT 三类主流数据源接入，覆盖实时数据采集主流场景。
  - **多数据格式支持**：数据源支持解析 CSV、JSON、Parquet 三种通用数据格式，适配多样化数据接入需求。
  - **Pipeline 数据流转**：支持创建数据流转 Pipeline，可对原始数据源数据进行过滤清洗，最终写入时序表，实现实时数据处理与落地。
  - **数据源查询**：新增 SHOW SOURCES 查询语句与系统表 information_schema.sources，支持快速查询所有已创建数据源；新增 SHOW CREATE SOURCE 语句，可直接查看数据源完整创建语句。
  - **流水线查询**：新增 SHOW PIPELINES 查询语句与系统表 information_schema.pipelines，支持批量查看所有已创建流水线；新增 SHOW CREATE PIPELINE 语句，便捷获取流水线创建配置语句。
- 新增 CREATE TABLE AS 建表语法，支持基于 SQL 查询结果直接快速建表，简化数据表创建流程。
- 新增 STRUCT 结构体、ARRAY 数组两种复合数据类型，适配复杂结构化数据存储与查询场景。

### 功能增强

- 认证能力拓展：服务端认证新增支持 chain 模式，丰富系统安全认证策略。
- 兼容性优化：优化 Postgres 兼容逻辑，大幅提升客户端连接适配兼容性。
- 查询性能提升：深度优化查询执行逻辑，有效提升整体数据查询效率。

### 问题修复

- 修复 JDBC 驱动写入数据时，无法正常获取 SQL 执行受影响行数的问题，保障数据写入结果可校验。
...
- **全文检索**：
  - 支持在 `STRING` 类型列上创建 `INVERTED INDEX`，可通过 `tokenizer`、`filters`、`with_position` 参数精细化配置索引行为；
  - 查询侧支持 `MATCH`、`QUERY`、`SCORE` 等全文检索函数，可执行关键词、短语和布尔表达式检索，并按相关性（BM25）评分排序；
  - 新增 `SHOW INDEXES` 语句，以及 `information_schema.indexes` 与 `information_schema.index_files` 系统表，便于统一查看全文索引、向量索引、主键及其索引文件信息。
- **数据批量操作能力**：
  - 新增 `INSERT INTO ... SELECT ...`，便于表间数据回填、迁移与批量转换。

### 功能增强

- **Prepared Statement 兼容性增强**：补齐 UDF、DATE 类型、事务相关语句等场景的 Prepared Statement 支持，提升预处理语句整体兼容性；
- **PostgreSQL 兼容性增强**：新增 SELECT current_catalog 语法支持，强化与 PostgreSQL 客户端及生态工具的适配性；
- **分布式查询诊断增强**：支持分布式 EXPLAIN 语句，可直观分析分布式查询的执行计划与路径，降低问题排查成本。

### 性能与体验优化

- **向量检索性能优化**：引入 adaptive filter 与 iterative ANN 等优化策略，在高过滤率场景下更好地平衡召回率与查询延迟;
- **时序查询性能优化**：优化 MemtableReader 读取逻辑，针对「时间序列数量多、单序列行数少」的场景减少无效数据扫描，提升读取效率；
- **可观测性**：优化日志格式及运行时日志输出内容，提升日志可阅读性。
...
- 修复 `TIMESTAMP` 数据类型转换时的整数溢出问题
- 修复插值过程中对 `NULL` 值处理不当的问题
...
- 修复 PIVOT 生成列数过多时可能导致异常的问题，现限制生成列总数不超过 4096
...
- 修复 append 模式下查询仍会错误过滤重复时间戳数据的问题，确保保留重复数据的场景下查询结果正确。
...
- 优化查询逻辑，提升部分场景的查询效率
- 向量检索能力升级，新增对 RQ 中向量转置的支持，并引入 SIMD 加速机制，IVF_RQ 检索模式的查询延迟显著降低 10%–40%，大幅提升检索响应速度

### 修复

- 修复 PromQL 查询语句中对英文句号 `.` 处理不正确的问题
...
- **PRQL 查询支持**：dlsql 命令行工具已集成 [PRQL](https://prql-lang.org/) 查询语言，为用户提供更具表达力的现代化数据查询与转换体验。具体用法请参阅 [dlsql 文档](../admin/datalayers-cli.md#交互语言)。
- **可视化 Web 控制台**：dlsql 命令行工具现已集成内置的 Web 控制台，提供数据库的可视化操作与管理界面，简化数据库管理任务。使用方法详见 [dlsql 文档](../admin/datalayers-cli.md#web-控制台)。
...
本版本重点增强了系统的安全性、可观测性、查询性能与向量检索能力，并进一步完善了对 Prometheus 生态的兼容性，为用户提供更灵活、更安全、更高性能的数据管理体验。

### 新功能

- **RBAC 访问控制**：新增 [基于角色的访问控制](../user-security/rbac/overview.md) 功能，实现细粒度的权限控制。
- **Prometheus 增强**：支持 Prometheus Remote Write 协议，可通过该协议直接写入数据。现 Datalayers 已完全兼容 Prometheus 写入与查询协议，可作为 Prometheus 的长期存储或替代方案。
- **向量索引**：新增多种 [向量索引](../vector-search/vector-index.md) 类型的支持，提升检索效率与灵活性。
- **用户认证**：新增 peer 认证机制，详见 [peer 认证](../admin/datalayers-cli.md)。
...
- **数据导出改进**
  - 优化 dldump 逻辑，提升导出效率，在导出大量数据时极大地降低了服务端内存占用。
- **启动效率提升**
  - 改进索引加载逻辑，提升索引加载的有效性。
...
- **混合缓存优化**：实现分布式混合缓存架构，极大提升缓存利用率、提升查询性能，同时支持缓存资源的水平扩展。
...
- 表名支持 `-` 连接符（如表名中包含 `-`，执行相关 SQL 语句时需使用反引号 `` ` `` 包裹对应字符串）。
