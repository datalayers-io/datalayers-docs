...
- Choose `tokenizer` based on log language (`standard` / `chinese`)
- `standard` tokenizes text by whitespace and punctuation and is suitable for English text
- Use `filters` to define the filter chain (for example, `lowercase,english_stop,english_stemmer`)
